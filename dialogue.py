...
class Decoder(nn.Block):
    ...
    def forward(self, cur_input, state, enc_states, character):
        ...
        input_and_c = nd.concat(self.embedding(cur_input), c, self.embedding_1(character[:,0]),
                                self.embedding_2(character[:,1]),self.embedding_3(character[:,2]),self.embedding_4(character[:,3]), dim=1)
...
def batch_loss(encoder, decoder, X, Y, M, loss):
    ...
    for epoch in range(num_epochs):
        ...
        for X, Y, M in data_iter:
            ...
            with autograd.record():
                l = batch_loss(encoder, decoder, X.as_in_context(ctx), Y.as_in_context(ctx), M.as_in_context(ctx), loss)
            l.backward()
            enc_trainer.step(1)
            dec_trainer.step(1)
            l_sum += l.asscalar()
