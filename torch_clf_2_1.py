...
all_set, seq_list, token_counter = get_data()
...
def data_loader(data_set, batch_size=8, max_l_word=300):
...
class ModelBase(nn.Module):
	def __init__(self, embedding=None, lm_model=False):
		...
		self.aug = False
		self.encode = TextEncoder(vocab_size=len(vocab), embedding_dim=1100, hidden_size=self.num_units, num_layers=2)
...
train_iter = data_loader(train_set, batch_size=16)
test_iter = data_loader(test_set, batch_size=32)
stack = []
...
model = train(net, train_iter, test_iter)
